...
use Andheiberg\Auth\Console\MakeRemindersCommand;
...
class ReminderServiceProvider extends ServiceProvider {
	...
	public function register()
	{
		...
		$this->registerEmailVerificationReminder();
	...
	{
		$this->app->bindShared('auth.password-reminder', function($app)
		{
			...
			$users = new \Andheiberg\Auth\EloquentUserProvider($this->app['hash'], $this->app['config']['auth.model']);

			$view = $app['config']['auth.password-reminder.email'];

			$subject = $app['config']['auth.password-reminder.subject'];
			...
			return new PasswordBroker(
				...
				$reminders, $users, $app['mailer'], new MessageBag, $view, $subject
	 ...
	 * @return void
	 */
	protected function registerEmailVerificationReminder()
	{
		$this->app->bindShared('auth.email-verification-reminder', function($app)
		{
			// The reminder repository is responsible for storing the user e-mail addresses
			// and password reset tokens. It will be used to verify the tokens are valid
			// for the given e-mail addresses. We will resolve an implementation here.
			...
			$view = $app['config']['auth.email-verification-reminder.email'];

			$subject = $app['config']['auth.email-verification-reminder.subject'];
	...
	public function provides()
	{
		return array(
			...
			'auth.password-reminder',
			'auth.email-verification-reminder'
		);
	}
